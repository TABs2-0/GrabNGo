...
class First_Page(Screen):
    def open_internal_website(self):
        # Ensure your main.py server is running!
        url = "http://127.0.0.1:8001" # Change port if main.py uses a different one
        webbrowser.open(url)
...
class Create_Buyer_Account_Page(Screen):
    ...
    def buyer_create_account(self):
        ...
        try:
            ...
            resp = requests.get(f"{BASE_URL}/buyer/id",params={"name": buyer_name})
...
class Create_Seller_Account_Page(Screen):
    ...
    # Use 'self' to access IDs directly within this screen
        try:
        ...
        # Add your account creation logic here (Firebase/DB)
            print(f"Data captured for {seller_name}")
        
        except KeyError:
            print("Error: One of the IDs is missing or misspelled in the KV file.")
        except AttributeError:
            print("Error: Could not access widget properties. Check your KV structure.")          
        try:
            ...
            if password_validator == 0:
                ...
                self.login_text = "Wrong password. Please enter the a strong Password"
                return 0
            ...
            print("Account has been created")
            self.login_text = "Account has been created"
            self.ids.seller_create_account_name.text = ""
            ...
            resp = requests.get(f"{BASE_URL}/seller/id",params={"name": seller_name})
            ...
            self.manager.current = "configurebuisnesspage"

        except Exception as e:
            print(e)
            self.login_text = "Error during Registration"
...
class BuyerDashboard(Screen):
    ...
    def load_sellers(self, dt):
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/sellers",params={"buyer_name": app.buyer_name})
...
class StorePage(Screen):
    ...
    def get_product_info(self, name):
        ...
        try:
            resp = requests.get(f"{BASE_URL}/product/detail", params={"p_name": name, "s_name": self.seller_name,"name":App.get_running_app().buyer_name})
            info = resp.json()
        except:
            info = [["N/A", "N/A", 0]]
...
class CartPage(Screen):
    ...
    def update_qty(self, ID, QTY):
        try:
            requests.post(f"{BASE_URL}/cart/update", json={"buyer_name": App.get_running_app().buyer_name,"location": App.get_running_app().buyer_location,"order_id": ID, "qty": QTY})
        except:
    ...
    def clear_cart(self):
        try:
            requests.post(f"{BASE_URL}/cart/clear", json={"buyer_name": App.get_running_app().buyer_name,"location": App.get_running_app().buyer_location,"order_id": ID, "qty": QTY})
        except:
...
class UpdateProductPopup(ModalView):
    p_name = StringProperty("")
    p_desc = StringProperty("")
    p_price = StringProperty("0")
    p_cat = StringProperty("")
    p_qty = StringProperty("0")
    ...
    def process_payment(self):
        ...
        try:
            ...
            for item in App.get_running_app().order_ids:
                requests.post(f"{BASE_URL}/delivery/populate", json={"order_id": item, "agent_name": agent_name,"name": App.get_running_app().buyer_name,"location":App.get_running_app().buyer_location})
            
            time_resp = requests.get(f"{BASE_URL}/delivery/time", params={"name": App.get_running_app().buyer_name,"location": App.get_running_app().seller_location})
    ...
    def submit_review(self, rat):
        ...
        try:
            ...
            for order in App.get_running_app().order_ids:
                requests.post(f"{BASE_URL}/order/finalize", json={"order_id": order, "date": date_str,"name": App.get_running_app().buyer_name})
...
class SellerProductCard(BoxLayout):
    ...
    def open_edit_popup(self):
        ...
        popup.p_price = str(self.p_price).replace("CFA ", "") 
        popup.p_cat = self.p_cat
        popup.p_qty = str(self.p_qty)
...
class Seller_Manager_Page(Screen):
    ...
    def load_seller_products(self):
        try:
            response = requests.get(
                f"{BASE_URL}/seller/inventory",
                params={"name": App.get_running_app().seller_name}
            )
            ...
            all_product_data = response.json()
            print(all_product_data)  # debug

            self.ids.seller_product_rv.data = all_product_data
    ...
    def update_product(self, name, des, cat, price, qty, img, Name, s_name):
        ...
        try:
            ...
            resp_id = requests.get(f"{BASE_URL}/product/id", params={"p_name": Name,"name":App.get_running_app().seller_name})
            ...
            payload = {
                ...
                "seller_id": App.get_running_app().seller_id,
                "name": App.get_running_app().seller_name
            }
    ...
    def get_image_and_upload(self,vps_ip, seller_id, product_name):
        ...
        def on_upload_pressed(instance):
            if file_chooser.selection:
                ...
                self.local_filename = os.path.basename(self.local_full_path)
                # Start Upload to VPS
                url = f"{BASE_URL}/upload/product"
                files = {'file': (self.local_filename, open(self.local_full_path, 'rb'), 'image/jpeg')}
...
class Delivery_Manager_Page(Screen):
    ...
    def load_agents(self):
        try:
            ...
            for agent in all_agents:
                ...
                loc_resp = requests.get(f"{BASE_URL}/agent/location", params={"agent_name": agent[1],"name":App.get_running_app().seller_name})
    ...
    def add_agent(self, name, location):
        ...
        try:
            ...
            requests.post(f"{BASE_URL}/seller/agent/add", json={
                "seller_name": App.get_running_app().seller_name,
                "agent_name": name,
                "location": location
            })
...
class ConfigureBuisnessPage(Screen):
    ...
    answer_to_password_forgotten = StringProperty("")
    text=StringProperty("Mobile\\FrontEnd\\Images\\logo.png")
    print("Tsuipppppppppppppp")
    new_path = StringProperty("")
    def update_profile_photo(self, vps_ip, seller_name):
        resp = requests.get(f"{BASE_URL}/seller/id",params={"name": seller_name})
        ...
        def on_upload_pressed(instance):
            if file_chooser.selection:
                ...
                url = f"{BASE_URL}/upload/profile_pic"
                ...
                try:
                    with open(full_path, 'rb') as f:
                        ...
                        if response.status_code == 200:
                            ...
                            self.ids.profile_image_widget.source = f"http://{vps_ip}:8000/{self.new_path}"
