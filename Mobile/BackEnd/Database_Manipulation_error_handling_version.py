...
class Display_From_Table:
    ...
    def display_seller_transactions(self, ID):
        try:
            ...
            print(f"Display Error: {e}")
            return None
# ONLY run this if I double-click THIS file directly.
# Do NOT run this if api.py or Buyer_Login.py imports it.
if __name__ == "__main__":
    try:
        # Create a temporary connection just for setup
        setup_con = Get_Database_connection()
        data = Create_Database(setup_con)
        setup_con.commit()
        print("Database setup complete.")
    except Exception as e:
        print(f"Main Loop Error: {e}")
    finally:
        if 'setup_con' in locals():
            setup_con.close()